...
async def get_channels(message: Message, session):
    telegram_id = message.from_user.id
    channels = await get_channels_from_db(telegram_id=telegram_id, session=session)
    await message.answer(f"Вот твои каналы", reply_markup=get_channels_menu(channels))
...
async def delete_current_channel(callback: CallbackQuery, session):
    channel_id = callback.data.split("_", 1)[1]
    # await callback.message.answer(text=f"{channel_id}")
...
async def add_channel(message: Message, session, state: FSMContext):
    telegram_id = message.from_user.id
